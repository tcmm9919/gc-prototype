            ...
            <TabsList className="flex h-auto flex-wrap justify-start gap-1 bg-transparent p-0">
              {TABS.map((t) => (
                <TabsTrigger
                  key={t.value}
                  value={t.value}
                  className="h-auto flex-none rounded-lg border-none bg-transparent px-4 py-2 text-[14px] font-medium text-muted-foreground transition-all hover:text-foreground data-[state=active]:bg-white data-[state=active]:text-zinc-900 data-[state=active]:shadow-[0_1px_2px_rgba(0,0,0,0.04),0_4px_16px_-2px_rgba(0,0,0,0.08)]"
                >
                  {t.label}
                </TabsTrigger>
              ))}
