  ...
  return (
    <div className="pb-6">
      <div className="grid items-start gap-6 lg:grid-cols-[336px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 rounded-2xl border border-border bg-card p-4 md:p-5">
          <div className="flex flex-col gap-4">
            {/* Информация об операции + Банковская */}
            <Card title="Информация об операции">
            ...
            <Card
              title={
                <span className="inline-flex items-center gap-2">
                  ...
                  Оповещения
                  {alerts.length > 0 ? <span className="ml-0.5 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-risk-medium/15 px-1.5 text-xs font-medium text-risk-medium tabular-nums">{alerts.length}</span> : null}
                </span>
              }
            >
              {alerts.length > 0 ? (
            ...
            <Card
              title={
                <span className="inline-flex items-center gap-2">
                  ...
                  Связанные кейсы
                  {cases.length > 0 ? <span className="ml-0.5 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-primary/15 px-1.5 text-xs font-medium text-primary tabular-nums">{cases.length}</span> : null}
                </span>
              }
            >
              {cases.length > 0 ? (
...
function Card({ title, children }: { title: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-xl border border-border p-4">
      <h4 className="mb-4 font-heading text-[15px] font-semibold">{title}</h4>
      {children}
    </div>
  )
}
