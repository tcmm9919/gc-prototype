...
export default function Strands({
  ...
  // Держим актуальные пропсы для requestAnimationFrame-цикла, не записывая ref в рендере.
  const propsRef = React.useRef(current);
  React.useEffect(() => {
  ...
  return (
    <div
      ref={ctnDom}
      className={cn("relative h-full w-full [&>canvas]:block [&>canvas]:h-full [&>canvas]:w-full", className)}
      style={style}
    />
  );
}
