...
`settings/risk-factors`. Подключены 4 состояния через `StateSwitch skeleton="table"`.
**Правка после ревью:** список переведён с кастомной div-grid таблицы на общий `<DataTable>` —
ровно как просил пользователь этой сессии («должен быть последовательным другим страницам с
таблицей»). Колонки (Название/Тип/Источник/Корзины/Вес·доля/Активен/Действия), мини-эквалайзер и
share-bar сохранены как cell-рендереры; убрана дублирующая контрол-полоса (её роль у тулбара
DataTable), inline-форма билдера осталась над таблицей (коммиты `1812d3e` → `033a8db`).

---

## Правки после ревью (advisor)
- **risk-factors → DataTable** (`033a8db`): единственная страница, что осталась на bespoke-таблице,
  переведена на общий компонент. Это и есть та консистентность, ради которой шёл прогон + прямая
  просьба пользователя. Мини-визуализации сохранены через `cell:`-рендереры.
- **ai → outline CTA** (`33e9851`): три primary-кнопки на тайлах нарушали «один primary CTA на экран».
- **workflow-detail** — сверен с `cases-detail-ref` по 3 измеримым пунктам (шапка→секция, `gap-4`
  между секциями, `p-5` секции): совпадает (единственный полный структурный rewrite — проверен особо).
